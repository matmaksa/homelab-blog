...
notes:
  ...
  - "Requires Headscale v0.29.3 command verification against the target instance before publication."
  - "Basis article should be aligned before this follow-up is published."
---

**Aktualisiert: August 2026 | Lesezeit: 10–12 Minuten**
...
| ✅ Geprüft gegen | Headscale-Dokumentation v0.29.3, Tailscale-Client-Befehle |
...
Der Client zeigt dir danach eine URL oder eine Auth-ID an. Die Auth-ID ist die einmalige Kennung dieses Anmeldevorgangs: Kopiere sie nur in den folgenden Befehl auf deinem Headscale-Server.
...
Suche die 100.x.x.x-Adresse von Gerät B und teste. Diese Adresse gehört zum privaten CGNAT-Adressbereich von Tailscale/Headscale; sie ist keine öffentliche Internetadresse:

```bash
ping 100.x.x.x
```

Falls MagicDNS aktiv ist, kannst du auch den Namen testen. MagicDNS bedeutet: Geräte erhalten im Headscale-Netz einen Namen, damit du nicht immer IP-Adressen merken musst:
...
Headscale nutzt das gleiche huJSON-Format wie Tailscale. huJSON ist JSON, in dem Kommentare erlaubt sind. In aktuellen Headscale-Versionen verweist du in der Konfiguration über `policy.path` auf diese Datei.
...
Vor dem Einrichten muss das Router-Gerät IPv4-Pakete weiterleiten können. Prüfe und aktiviere das dort zuerst:

```bash
sudo sysctl -w net.ipv4.ip_forward=1
echo 'net.ipv4.ip_forward=1' | sudo tee /etc/sysctl.d/99-headscale-subnet-router.conf
sudo sysctl --system
```

Die erste Zeile wirkt sofort; die Datei unter `/etc/sysctl.d/` macht die Einstellung nach einem Neustart dauerhaft. Führe diesen Schritt nur auf dem vorgesehenen Subnet-Router aus, nicht auf jedem Client.
...
Dann hast du wahrscheinlich zu streng gestartet. Setze kurzfristig eine Admin-Regel für dein Hauptgerät und teste danach schrittweise enger. Dafür muss die Gruppe zuerst definiert sein:
